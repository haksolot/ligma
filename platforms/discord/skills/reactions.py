...
class ReactionSkill(BaseSkill):
    ...
    async def execute_action(self, response: str, message: discord.Message) -> Tuple[str, Dict[str, Any]]:
        # Robustly find all reactions even with typos like REAACT or lowercase
        react_matches = re.findall(r'\[RE+A+CT:?\s*(.*?)\]', response, re.IGNORECASE)
        ...
        cleaned_response = re.sub(r'\[RE+A+CT:?.*?\]', '', response, flags=re.IGNORECASE)
